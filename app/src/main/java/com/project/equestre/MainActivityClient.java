...
public class MainActivityClient extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener,
        ReserverSeance.Ireservation, AbonnementAnnuel.Iabonnement, Profile.IProfile, F_C_Schedule.ICSchedule
        , F_Get_Notes.IGetNotes, Add_Note.IAddNote {
    ...
    public boolean onNavigationItemSelected(@NonNull MenuItem menuItem) {
        ...
        switch (menuItem.getItemId()){
            ...
            startActivity(new Intent(MainActivityClient.this,Login.class));  break;
            case R.id.abonnement: loadFragment(new AbonnementAnnuel()); break;
            case R.id.reserver_seance: loadFragment(new ReserverSeance()); break;
            case R.id.profile: loadFragment(new Profile()); break;
            case R.id.schedule: loadFragment(new F_C_Schedule()); break;
    ...
    @Override
    public void reservationConfirmed(final String s) {
            StringRequest request = new StringRequest(Request.Method.POST, URL+"reservationClient.jsp", new Response.Listener<String>(){
                @Override
                public void onResponse(String s) {
                    if(s.equals("ok"))
                    Toast.makeText(MainActivityClient.this,"reservation successful",Toast.LENGTH_LONG).show();
                }
            },new Response.ErrorListener(){
                @Override
                public void onErrorResponse(VolleyError volleyError) {
                    Toast.makeText(MainActivityClient.this, "Some error occurred -> "+volleyError, Toast.LENGTH_LONG).show();;
                }
            }) {
                @Override
                protected Map<String, String> getParams() throws AuthFailureError {
                    ...
                    parameters.put("reservation", s);
                    parameters.put("ID_user", String.valueOf(userLocalStorage.getLoggedInUser().getId_user()));
                    return parameters;
                }
            };

            RequestQueue rQueue = Volley.newRequestQueue(MainActivityClient.this);
            rQueue.add(request);
    ...
    @Override
    public void abonnementRenouvler() {
        StringRequest request = new StringRequest(Request.Method.POST, URL+"abonnementClient.jsp", new Response.Listener<String>(){
            @Override
            public void onResponse(String s) {
                if(s.equals("ok"))
                    Toast.makeText(MainActivityClient.this,"abonnement successful",Toast.LENGTH_LONG).show();
            }
        },new Response.ErrorListener(){
            @Override
            public void onErrorResponse(VolleyError volleyError) {
        ...
        StringRequest request = new StringRequest(Request.Method.POST, URL+"editProfile.jsp", new Response.Listener<String>(){
            ...
            public void onResponse(String s) {
                ...
                try {
                    ...
                    if(json.has("error")){
                        Toast.makeText(MainActivityClient.this,json.getString("error"),Toast.LENGTH_LONG).show();

                    }else {
                        Toast.makeText(MainActivityClient.this, "Profile Modification Successful", Toast.LENGTH_LONG).show();
                        ...
                        loadFragment(new MainFragmentClient());
                    }
    ...
    @Override
    public void getSchedule(final View vi, final String time) {


        //  Toast.makeText(Login.this, "click", Toast.LENGTH_LONG).show();
        StringRequest request = new StringRequest(Request.Method.POST, URL+"getScheduleC.jsp", new Response.Listener<String>(){
            @RequiresApi(api = Build.VERSION_CODES.O)
            @Override
            public void onResponse(String s) {
                ...
                try {
                    ...
                    if(!listJson.isEmpty() && listJson.get(0).has("error")){
                        Toast.makeText(MainActivityClient.this,listJson.get(1).getString("error"),Toast.LENGTH_LONG).show();

                    }else {
                        Toast.makeText(MainActivityClient.this, "Getting Schedule Successful", Toast.LENGTH_LONG).show();
                        ...
                        ll.setAdapter(new ScheduleAdapter(vi.getContext(),R.layout.item_c_schedule, pendingScheduleLocalStorage.getSchedule()));
